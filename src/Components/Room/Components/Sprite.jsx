...
export default function Sprite({ spid = 0 }) {
    ...
    const spriteObj = new SpriteObject(spid);

    spriteObj.pre_load(girl.id, device)
    ...
    //                 document.getElementById(`sp-body-${spid}-0`).style.opacity = 1;
    ...
    //     actual_body = newb;
    //     document.getElementById(`sp-body-${spid}-${newb}`).src = conj_data.b[body];
    //     Object.keys(style_body_actual).map(st => document.getElementById(`sp-body-ctn-${spid}-${newb}`).style[st] = style_body_actual[st])

    //     document.getElementById(`sp-body-${spid}-${newb}`).onload = () => {
    //         document.getElementById(`sp-body-${spid}-${newb}`).style.opacity = 1;
    //         setTimeout(() => {
    //             document.getElementById(`sp-body-${spid}-${oldb}`).style.opacity = 0;
    //             Object.keys(style_body_hidden).map(st => document.getElementById(`sp-body-ctn-${spid}-${oldb}`).style[st] = style_body_hidden[st])
    ...
    useEffect(() => {
        ...
        spriteObj.update_both(spriteObj.head, spriteObj.body)
    }, [])




    return <div style={{ position: "fixed", bottom: spriteObj.conj_style.bot_ctn, left: spriteObj.conj_style.lf_ctn, textAlign: "center" }}>
        {/* HEAD 1 */}
        <div style={(spriteObj.actual_head != 0) ? (spriteObj.style_head_hidden) : (spriteObj.style_head_actual)} id={`sp-head-ctn-${spid}-0`}>
            <img className="w-100 h-100" id={`sp-head-${spid}-0`} style={{ opacity: 0, transition: "opacity .3s" }} alt="" /></div>
        {/* HEAD 2*/}
        <div style={(spriteObj.actual_head != 1) ? (spriteObj.style_head_hidden) : (spriteObj.style_head_actual)} id={`sp-head-ctn-${spid}-1`} >
            <img className="w-100 h-100" id={`sp-head-${spid}-1`} style={{ opacity: 0, transition: "opacity .3s" }} alt="" /></div>



        {/* BODY */}
        <div id={`sp-body-ctn-${spid}-0`} style={(spriteObj.actual_body != 0) ? (spriteObj.style_body_hidden) : (spriteObj.style_body_actual)} >
            <img className="w-100 h-100" id={`sp-body-${spid}-0`} style={{ opacity: 0, transition: "opacity .3s" }} alt="" /></div>


        <div id={`sp-body-ctn-${spid}-1`} style={(spriteObj.actual_body != 1) ? (spriteObj.style_body_hidden) : (spriteObj.style_body_actual)}>
            <img className="w-100 h-100" id={`sp-body-${spid}-1`} style={{ opacity: 0, transition: "opacity .3s" }} alt="" /></div>
    </div>
}
